...
Usage:
perl $0 -r <reference fasta> -a <alignment file> -f <0=sam, 1=bam> -d <dindel path> -t <max threads> -v <variant file> -l <library file>
...
my ($ref_fasta, $alignment, $isbam, $pipath, $dipath, $insertsz, $maxthreads, $ref_ver, $lib_file, $var_file);
GetOptions(
"r=s"=>\$ref_fasta,
"a=s"=>\$alignment,
"f=i"=>\$isbam,
...
die "$usage" if(!$ref_fasta || !$alignment || !$dipath  || !$maxthreads || !$var_file || !$lib_file); 
...
print "varfile: $var_file \n";
print "lib file: $lib_file \n";
...
#check index file
#if (!(-e "$bamfile.bai") || !(-e "$bamfile.bai")){
...
runcmd($cmd);
START:
$cmd = "python $dipath/makeWindows.py --inputVarFile $var_file --windowFilePrefix $fileprefix.di.window --numWindowsPerFile 1000";
runcmd($cmd);
...
for (my $i = 0; $i <= $#dircontent; ++$i){
	if ($dircontent[$i] =~ /^$shortfileprefix.di.window.[0-9]{1,4}.txt$/){
		...
		$cmd = "$dipath/dindel --analysis indels --doDiploid --bamFile $bamfile.sorted.bam --ref $ref_fasta --varFile $workingdir/".$dircontent[$i]." --libFile $lib_file --outputFile $workingdir/".$dircontent[$i].".prc >> $fileprefix.windprc_log";
		#$cmd = "$dipath/dindel --analysis indels --doPooled --bamFiles bamfiles --ref $ref_fasta --varFile $workingdir/".$dircontent[$i]." --libFile $fileprefix.candidate.di.libraries.txt --outputFile $workingdir/".$dircontent[$i].".prc >> $fileprefix.windprc_log";
		#$cmd = "$dipath/dindel --analysis indels --doPooled --bamFiles bamfiles --ref $ref_fasta --varFile $workingdir/".$dircontent[$i]." --libFile $fileprefix.candidate.di.libraries.txt --outputFile $workingdir/".$dircontent[$i].".prc >> $fileprefix.windprc_log";
		#/home/wkljohn/Programs/indel_caller/dindel/dindel --analysis indels --doPooled --bamFiles /store/wkljohn/alansample/bamfiles --ref /data/genome/hg19/ucsc.hg19.fasta --varFile /store/wkljohn/alansample/merged/all.di.window.1.txt  --libFile /store/wkljohn/alansample/all.libraries.txt  --outputFile /store/wkljohn/alansample/tmp
...
die "Completed window, manual merge call!\n";
